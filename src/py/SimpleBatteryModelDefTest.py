...
class SimpleBatterySimEnv(gym.Env):
    ...
    def __init__(self, LMP_file, istartday):
        ...
        self.maxPt = 1.0
        self.chargeBoundaryPenalty = 3000
        self.orgSOCbiaspenalty = 3000
        self.simuhours = 0
        self.simudays = 3
    ...
    def _reset(self):
        ...
        self.simustartday = 2 #np.random.randint(0,10) # an integer, in be in the preset days?
        self.currentLMP = self.LMP_days[self.simustartday, 0]
    ...
    def _validate(self, istartday):
        ...
        for i in range (0, observation_Et_length):
            self.observation_Et_queue.append(self.BatteryEt)
            self.observation_Pt_queue.append(self.BatteryPt)
            self.observation_LMP_pastoneday_queue(self.currentLMP)   # this may cause confuse for the AI????
